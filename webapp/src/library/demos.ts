/**
 * The four real, hand-built simulators this course ships, mounted inside
 * the "Live Demo" tab of the specific module they prove, instead of living
 * in a separate top-level tab strip disconnected from the module they
 * belong to. Each function renders into the container the library detail
 * pane hands it; internal element lookups stay as plain document.getElementById
 * calls since only one demo is ever mounted at a time.
 */
...
// ---------------------------------------------------------------------------
// Module 1 (mem_addressing): virtual-to-physical address translation
// ---------------------------------------------------------------------------
...
export function renderPagingDemo(container: HTMLElement): void {
  container.innerHTML = `
    <div class="card">
      ...
      <p class="desc">
        ...
        every access, a 9-bit index into each of four table levels,
  ...
  function run() {
    ...
    try {
      ...
      document.getElementById('verdict-card')!.innerHTML = `<div class="card"><p class="desc" style="color:var(--danger)">Not a valid hex address.</p></div>`;
      return;
    }
    ...
    document.getElementById('va-fields-card')!.outerHTML = `
      <div class="card" id="va-fields-card">
        ...
        <p class="desc">This split is identical for every process, it's fixed by the MMU hardware, not by any OS policy.</p>
        ...
        <div class="verdict ${same ? 'same' : 'diff'}">
          ${
            same
              ...
              : `Confirmed: the identical virtual address <code>${hex(big)}</code> resolves to <strong>different physical RAM</strong> in each process. Nothing about the number itself pins it to a location, only that process's own page tables do.`
          }
        </div>`;
    } else {
      verdict.innerHTML = `
        <div class="verdict diff">
          At least one process has no mapping for this address at all, a real, valid outcome. Isolation cuts both ways: a process can't even discover whether another process has something mapped there.
        </div>`;
    }
  }
...
// ---------------------------------------------------------------------------
// Module 2 (privilege_rings): privilege rings
// ---------------------------------------------------------------------------
...
export function renderRingsDemo(container: HTMLElement): void {
  container.innerHTML = `
    <div class="card">
      ...
      <p class="desc">
        ...
        the CPU's own decode logic allows to run, from Module 2
      ...
      <div class="ring-layout">
        ...
        <div class="ring-info">
          <p class="desc">
            ...
            ${selectedRing === 0 ? ', kernel mode. Full instruction set, unrestricted memory access.' : ''}
            ${selectedRing === 3 ? ', user mode. Where every browser renderer process runs; no privileged instruction executes here.' : ''}
            ${selectedRing === 1 || selectedRing === 2 ? ', architecturally real, but unused by every mainstream OS (no CPL check below treats these specially here).' : ''}
          </p>
...
// ---------------------------------------------------------------------------
// Module 16 (http_parsing): raw bytes -> parsed HTTP response
// ---------------------------------------------------------------------------
...
export function renderHttpDemo(container: HTMLElement): void {
  container.innerHTML = `
    <div class="card">
      ...
      <p class="desc">
        ...
        no framework, a status line, headers, a blank line, then a body framed
  ...
  container.querySelectorAll<HTMLButtonElement>('.example-btn').forEach((btn) => {
...
// ---------------------------------------------------------------------------
// Modules 19 & 20 (html_tokenizer, dom_tree_builder): HTML -> tokens -> DOM
// ---------------------------------------------------------------------------
...
export function renderDomDemo(container: HTMLElement): void {
  container.innerHTML = `
    <div class="card">
      ...
      <p class="desc">
        ...
        that stream into a tree, including the real, spec-accurate recovery behavior for
...
/** Maps a module's id (from library.generated.ts) to the demo that proves
 * it, so the library's "Live Demo" tab can mount the right one without the
 * two systems (module list vs. interactive tabs) living apart on the page. */
export const DEMO_BY_MODULE_ID: Partial<Record<string, (container: HTMLElement) => void>> = {
  mem_addressing: renderPagingDemo,
  privilege_rings: renderRingsDemo,
  http_parsing: renderHttpDemo,
  html_tokenizer: renderDomDemo,
  dom_tree_builder: renderDomDemo,
};
